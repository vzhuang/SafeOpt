...
def run_trial(args):
    ...
    for function in range(num_functions):
        ...
        kernel2 = GPy.kern.Matern32(input_dim=len(bounds), variance=50.,
                                    lengthscale=0.25, ARD=False)
        kernel3 = GPy.kern.Matern32(input_dim=len(bounds), variance=50.,
                                    lengthscale=0.5, ARD=False)        
        def sample_safe_fun():
            safe_seeds = []
            ...
            try:
                ...
                for t in range(T):
                    ...
                    for p in parameter_set: 
                        ...
                        s_std = np.sqrt(s_var)
                        p_safe = (1 - norm.cdf((thresh-s_mean)/s_std))
                        if p_safe > max_pr:
                            max_pr = p_safe
                            max_xpr = p
                        if p_safe > 0.999:
                        ...
                        CEI = (1 - norm.cdf((thresh-s_mean)/s_std)) * u_std * \
                ...
                gp = GPy.models.GPRegression(x0, y0[:, 0, None], kernel,
                                             noise_var=noise_var)
                gp2 = GPy.models.GPRegression(x0, y0[:, 1, None], kernel3,
