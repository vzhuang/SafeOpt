...
class SafeStage(GaussianProcessOptimization):
    """A class for Safe Bayesian Optimization.

    This class implements the `SafeStage` algorithm. It uses a Gaussian
    ...
    >>> opt = SafeStage(gp, parameter_set, fmin=[0.])
    ...
    def use_lipschitz(self):
        ...
        By default this is set to False, which means the adapted SafeStage
    ...
    def compute_sets(self, full_sets=False):
        ...
        full_sets: boolean
            ...
            computations that are not relevant for running SafeStage
        ...
        if full_sets:
            ...
            s = np.logical_and(self.S, self.S)
        ...
        for index in sort_index:
            ...
            # uncertain element gets picked by SafeStage anyways, we can
    ...
    def get_new_query_point(self, ucb=False, exploit=False):
        """
        ...
        if exploit:
            u = self.Q[:, 1]
            value = np.max(u[self.S])
            x = self.inputs[self.S, :][np.argmax(value), :]
            return x
        ...
        if ucb:
            ...
            MG = self.G #np.logical_or(self.M, self.G)
            if np.sum(self.G) == 0:
                MG = self.M
    ...
    def optimize(self, context=None, ucb=False, exploit=False):
...
class SafeOpt(GaussianProcessOptimization):
    ...
    def get_new_query_point(self, ucb=False):
        ...
        if ucb:
            ...
            l = self.Q[:, ::2]
            u = self.Q[:, 1::2]
            #print(np.sum(self.M), np.sum(self.G))
