...
def run_trial(args):
    ...
    for function in range(num_functions):
        ...
        kernel = GPy.kern.Matern52(input_dim=len(bounds), variance=0.5,
                                   lengthscale=0.5, ARD=False)
        kernel2 = GPy.kern.Matern52(input_dim=len(bounds), variance=5.,
                                    lengthscale=0.2, ARD=False)
        kernel3 = GPy.kern.Matern52(input_dim=len(bounds), variance=5.,
                                    lengthscale=0.4, ARD=False)
        kernel4 = GPy.kern.Matern52(input_dim=len(bounds), variance=5.,
